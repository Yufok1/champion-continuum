...
Tools uses direct `native.*` relay commands when the MCP sidecar is unavailable,
so hosted/tool-less agents get a real answer instead of a zero-tool dead end. It
also tries to self-index the local MCP sidecar at `http://127.0.0.1:7872/mcp/sse`
...
`start_all_hf_provider_daemons.bat` starts the curated scout pack of verified
chat routes supported by the installed Hugging Face client. Routes observed as
account-credit-gated stay out of the default pack. The processes do not use
credits until they are assigned a turn and call a provider.
...
.\start_all_hf_provider_daemons.bat --include-unverified
```
...
Use `--include-unverified` only when you want to try additional chat-capable
catalog providers. Known client-unsupported providers are still skipped; manual
or credit-gated routes may fail and report their own auth, billing, or
availability error.
